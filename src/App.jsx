...
export default function App() {
  ...
  function openReport() { if (!session) { setAuthOpen(true); return } setPicked(null); setReportOpen(true) }
  function mapClick(coordinates) { if (!session) { setAuthOpen(true); return } setPicked(coordinates); setReportOpen(true) }
  ...
  if (!session && authOpen) return <AuthPage theme={theme} language={language} onToggleLanguage={toggleLanguage} onToggleTheme={toggleTheme} onEmployeeLogin={() => setEmployeeLogin(true)} onAuthenticated={(nextSession) => { setSession(nextSession); setAuthOpen(false) }} />
  ...
  return <div className="app-shell">{demoMode && <AlertBanner onDismiss={() => setDemoMode(false)} language={language} />}<Navbar onSearch={search} onAuth={!session ? () => setAuthOpen(true) : undefined} onEmployeeDashboard={session?.is_employee ? () => setEmployeeView(true) : undefined} onSignOut={session ? () => { signOut(); setSession(null) } : undefined} theme={theme} onToggleTheme={toggleTheme} language={language} onToggleLanguage={toggleLanguage} /><NotificationSystem alertLevel={alertLevel} language={language} /><main className="layout"><Sidebar incidents={incidents} language={language} onReport={openReport} demoMode={demoMode} onDemoToggle={() => setDemoMode((value) => !value)} lastUpdated={lastUpdated} onRefresh={refresh} loading={loading} /><section className="map-area"><MapView incidents={incidents} language={language} target={target} onMapClick={mapClick} />{error && <div className="map-error"><AlertCircle size={18} /> {error}</div>}<button className="fab" onClick={openReport} aria-label={language === 'ne' ? 'जोखिम रिपोर्ट गर्नुहोस्' : 'Report a hazard'}><Plus size={22} /></button></section></main>{reportOpen && <ReportModal coordinates={picked} language={language} onClose={() => setReportOpen(false)} onSubmit={submit} />}</div>
}
